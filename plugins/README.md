...
| [codex-oauth](./codex-oauth/) | OpenAI Codex integration with secure OAuth 2.0 authentication | **Commands:** `/codex` - Query Codex, `/codex-config` - Configure auth, `/codex-clear` - Clear credentials<br>**MCP Server:** 5 tools for programmatic access (codex_query, codex_status, codex_login, codex_clear, codex_models)<br>**Skill:** Auto-activation for Codex-related queries |
